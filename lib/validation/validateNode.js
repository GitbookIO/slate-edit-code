        ...
        validate(node) {
            ...
            nodes.forEach(child => {
                if (child.kind === 'text') {
                    ...
                    toReplace.push([child, lines]);
                } else if (child.type !== opts.lineType) {
                    toRemove.push(child);
                }
            });

            if (toReplace.length || toRemove.length) {
                return { toReplace, toRemove };
            }
            return null;
        },

        normalize(change, node, { toReplace, toRemove }) {
            toReplace.forEach(([child, nodes]) => {
                const baseIndex = node.nodes.indexOf(child);
