...
/**
 * @return {Object} A rule that ensure code blocks only contain lines of code.
 */
function onlyLine(opts) {
    return {
        ...
        validate(node) {
            ...
            nodes.forEach(node => {
                if (node.kind === 'text') toWrap.push(node);
                if (node.type !== opts.lineType) toRemove.push(node);
            });

            if (toWrap.length == 0 && toRemove.length == 0) {
                return null;
            }
